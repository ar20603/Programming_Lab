...
class Calculator extends JFrame implements ActionListener, KeyListener
{
	...
	//  true - choosing number key, false - choosing function key
	private static boolean choosing_operand = true;
	...
	public void keyPressed(KeyEvent ke) {
		...
		if(ke.getKeyCode() == KeyEvent.VK_ENTER) {
			...
			if(choosing_operand) {
				// for any number key, just append in the textbox
				textDisplay.setText(textDisplay.getText() + numButtons[active_num_index].getText());

				// toggling this boolean, to choose function key next
				choosing_operand = false;
				active_func_index = 0;

				// updating colors of active and last active keys
				...
				if(funcButtons[active_func_index].getText().equals("Clear")) {
					...
					// toggling this boolean, to choose operator/number key next
					choosing_operand = true;
					active_num_index = 0;
			...
			// toggling between choosing operator and operand
	...
	public static void main(String[] args) {
		...
		// colouring the initial active key
		numButtons[active_num_index].setBackground(Color.CYAN);
		...
		ActionListener changeActiveButton = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
				...
				if(choosing_operand) {
					// if choosing operand / number key
					...
					// if choosing operator / function key)
